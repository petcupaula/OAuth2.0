...
def showLogin():
    ...
    #return "The session state: %s" %login_session['state']
...
def gconnect():
    ...
    # see if user exists, otherwise create new one
...
def fbconnect():
    ...
    url = 'https://graph.facebook.com/oauth/access_token?grant_type=fb_exchange_token&client_id=%s&client_secret=%s&fb_exchange_token=%s&access_token=%s' % (
    ...
    url = 'https://graph.facebook.com/v7.0/me?access_token=%s&fields=name,id,email' % token
    h = httplib2.Http()
    result = h.request(url, 'GET')[1]
    ...
    url = 'https://graph.facebook.com/v7.0/me/picture?access_token=%s&redirect=0&height=200&width=200' % token
    h = httplib2.Http()
    result = h.request(url, 'GET')[1]
    data = json.loads(result)
